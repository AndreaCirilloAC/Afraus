...
percent_distr           <- data.frame(table(distr_digits)
                                      /length(distr_digits))
...
#based on Nigrini (1996), calculate a z-statistic in order to 
#assess the statistical significance of deviations
...
# compute z-statistic
test_table$z <- (abs(test_table$real_freq-test_table$benf_freq)
                 -1/(2*nrow(data)))/dev
...
data           <- data.frame(data[,2:4],data$pvalue) 
colnames(data) <- c("date","value","test","benford_significance") 
