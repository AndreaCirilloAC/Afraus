...
percent_distr           <- data.frame(table(distr_digits)/length(distr_digits)*100)
...
digit      <- seq(1:9)
benf_freq  <- round((log(1+(1/ digit),10))*100,4)
benf_distr <- data.frame(digit,benf_freq)
test_table <- merge(benf_distr,percent_distr, by="digit",all.x=TRUE) 


# for each number calculate the difference between the real and ideal distribution
test_table$difference <- test_table$benf_freq - test_table$real_freq
